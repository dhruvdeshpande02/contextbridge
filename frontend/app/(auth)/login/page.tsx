...
/* Floating node dot */
function Node({ x, y, delay, r = 4 }: { x: number; y: number; delay: string; r?: number }) {
  return (
    <circle cx={x} cy={y} r={r} fill="#4f7ef8" opacity="0.35"
      className="animate-pulse-glow" style={{ animationDelay: delay }} />
  );
}

/* Animated neural background — SVG canvas */
function NeuralCanvas() {
  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 380 580" preserveAspectRatio="xMidYMid slice">
      ...
      {nodes.map((n, i) => (
        <Node key={i} x={n.x} y={n.y} delay={`${(i * 0.25) % 2}s`} r={i % 3 === 0 ? 5 : 3} />
      ))}
    </svg>
  );
}
...
export default function LoginPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen overflow-hidden">
      ...
      {/* Left — animated branding panel (desktop only) */}
      ...
      {/* Right — form with matching ambient animation */}
      <div className="relative flex w-full md:w-1/2 flex-col items-center justify-center px-6 md:px-16 py-12 md:py-0 overflow-hidden">
        ...
        {/* Mirrored ambient background — sparser, more subtle */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 380 580" preserveAspectRatio="xMidYMid slice">
        ...
        {/* Floating rings — mirror positions */}
        {[
        ...
        <div className="relative z-10 w-full max-w-sm animate-slide-up">
          ...
          <p className="text-xs font-semibold uppercase tracking-widest text-muted mb-6">Sign In</p>
          ...
          <p className="mt-5 text-xs text-muted">
            New?{" "}
            <Link href="/register" className="text-subtle hover:text-ink underline underline-offset-2 transition-colors">
              Create account
            </Link>
          </p>
        </div>
